...
LCDWIKI_TOUCH::LCDWIKI_TOUCH(int8_t tcs, int8_t tclk, int8_t tdout, int8_t tdin,
                             int8_t tirq) {
...
uint16_t LCDWIKI_TOUCH::TP_Read_XY(uint8_t xy) {
  ...
  for (i = LOST_VAL; i < READ_TIMES - LOST_VAL;
       i++)  // Remove maximum and minimum values
  {
    sum += buf[i];
  }
...
uint8_t LCDWIKI_TOUCH::TP_Read_Coordinate2(uint16_t *x, uint16_t *y) {
  ...
  if (((x2 <= x1 && x1 < x2 + ERR_RANGE) ||
       (x1 <= x2 && x2 < x1 + ERR_RANGE))  // Before and after the two samples
                                           // are within +- ERR_RANGE.
...
void LCDWIKI_TOUCH::TP_Set_Rotation(uint8_t val) { touch_rotation = val; }

uint8_t LCDWIKI_TOUCH::TP_Get_Rotation(void) const { return touch_rotation; }

void LCDWIKI_TOUCH::LCD_Set_Rotation(uint8_t val) { lcd_rotation = val; }

uint8_t LCDWIKI_TOUCH::LCD_Get_Rotation(void) const { return lcd_rotation; }

void LCDWIKI_TOUCH::TP_Set_State(uint8_t val) { touch_statue = val; }

uint8_t LCDWIKI_TOUCH::TP_Get_State(void) const { return touch_statue; }
